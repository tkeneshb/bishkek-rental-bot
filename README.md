...
1. **Claude reads the text** and decides whether it was written by an
   ...
   confidence is ≥ 0.85 and it's flagged as a realtor — blocked. Between
   0.5 and 0.85 — goes to the moderator, because falsely blocking a real
   owner is worse than letting a realtor through.
...
5. **pHash on photos** — I implemented DCT-based hashing from scratch
   using Pillow. The image is downscaled to 32×32 grayscale, a 2D DCT is
   applied, the top-left 8×8 block is taken — that gives a 64-bit
   fingerprint. Comparison via Hamming distance, threshold 8. Distance
   ≤ 3 — hard block (clearly the same photos), 4–8 — moderation. DCT is
   resistant to resize, compression, and brightness changes, so realtors
   can't just re-export the image and call it new.
...
- Claude sometimes tags real owners as realtors when they write in a dry,
  matter-of-fact style. I rewrote the prompt three times, it's still not
  perfect. Probably needs to be fine-tuned on collected examples, but
  that's a separate project.
- SQLite handles the current load fine, but at thousands of listings per
  day it'll hit write-lock issues. PostgreSQL is the next step.
